...
//  ZMTP 1.0 short frame
typedef struct {
    byte size;              //  Size, 1 to 255
    byte flags;             //  Must be zero
    byte data [254];        //  Message data
} zmtp10_msg_t;
...
static void
zmtp10_recv (int handle, zmtp10_msg_t *msg)
{
    tcp_recv (handle, (byte *) msg, 1);
    if (msg->size == 0xff) {
        byte encoded [8];
        tcp_recv (handle, encoded, 8);
        //  We'd normally ntoh all 8 bytes
        msg->size = encoded [7];   
    }
    tcp_recv (handle, (byte *) msg + 1, msg->size);
}

static void
zmtp10_send (int handle, zmtp10_msg_t *msg)
{
    tcp_send (handle, (byte *) msg, msg->size + 1);
}
